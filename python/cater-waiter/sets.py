...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for category, name in categories:
        if ingredient_set.issubset(category):
...
def tag_special_ingredients(dish):
    ...
    return dish[0], ingredients & set(SPECIAL_INGREDIENTS)
...
def compile_ingredients(dishes):
    ...
    ingredients = set()
    for dish in dishes:
        ingredients = ingredients | set(dish)
...
def singleton_ingredients(dishes, intersection):
    ...
    all_ingredients = set([item for dish in dishes for item in dish])

    for i, dish1 in enumerate(dishes):
        for j, dish2 in enumerate(dishes):
            if i != j:
                all_ingredients = all_ingredients - (set(dish1) & set(dish2))
        
    return all_ingredients - set(intersection)
