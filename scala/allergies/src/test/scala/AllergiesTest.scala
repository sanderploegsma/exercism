...
class AllergiesTest extends FunSuite with Matchers {
  ...
  test("allergic to just eggs") {
    Allergies.list(1) should contain(Allergen.Eggs)
  }

  test("allergic to just peanuts") {
    Allergies.list(2) should contain(Allergen.Peanuts)
  }

  test("allergic to just strawberries") {
    Allergies.list(8) should contain(Allergen.Strawberries)
  }

  test("allergic to eggs and peanuts") {
    Allergies.list(3) should contain only(Allergen.Eggs, Allergen.Peanuts)
  }

  test("allergic to more than eggs but not peanuts") {
    Allergies.list(5) should contain only(Allergen.Eggs, Allergen.Shellfish)
  }

  test("allergic to lots of stuff") {
    Allergies.list(248) should contain only(
  ...
  test("allergic to everything") {
    Allergies.list(255) should contain only(
      Allergen.Eggs,
      Allergen.Peanuts,
      Allergen.Shellfish,
      Allergen.Strawberries,
  ...
  test("ignore non allergen score parts") {
    Allergies.list(509) should contain only(
      Allergen.Eggs,
      Allergen.Shellfish,
      Allergen.Strawberries,
      Allergen.Tomatoes,
